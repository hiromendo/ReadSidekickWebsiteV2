    ...
    >
      ...
      <div className="editorial-container relative z-10">
        ...
        <div ref={headerRef} className="mb-16 md:mb-24 max-w-3xl">
          ...
          >
            {t.howItWorks.title}{' '}
            <span className="italic">{t.howItWorks.titleItalic}</span>
          </motion.h2>
        ...
        >
          ...
          <div className="p-6 md:p-8 lg:p-10">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {/* Mode description */}
              <div>
                <div className={`inline-block px-3 py-1 ${modes[activeMode].color} text-white text-xs font-mono rounded-full mb-4`}>
                ...
                {activeMode === 'hard' && (
                  <div className="bg-coral-50 border border-coral-200 rounded-lg p-4">
                    <p className="font-mono text-body-sm text-coral-800 leading-relaxed">
                      <strong>{t.howItWorks.modes.hard.teachNote}</strong>
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="bg-ivory-50 rounded-xl p-6 border border-ink-800/5">
                <span className="editorial-label text-ink-700/50 block mb-3">{t.howItWorks.sampleOutput}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {(Object.keys(modes) as Mode[]).map((mode, index) => (
            ...
            >
              <div className={`w-3 h-3 ${modes[mode].color} rounded-full mb-4`} />
              <h3 className="font-serif text-xl text-ink-900 mb-2">{modes[mode].title}</h3>
          ...
          <ul className="space-y-3">
            {comingSoon.map((feature) => (
              <li key={feature} className="flex items-start gap-3">
                ...
                <span className="font-mono text-body-sm text-ink-700/80">{feature}</span>
              </li>
            ))}
          </ul>
        </motion.div>
